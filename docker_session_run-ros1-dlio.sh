...
IMAGE_NAME='dlio_noetic'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/ros_ws/dataset/recorded-dlio.bag'
...
roslaunch direct_lidar_inertial_odometry dlio.launch pointcloud_topic:='"$POINTCLOUD_TOPIC"' imu_topic:='"$IMU_TOPIC"' use_sim_time:=true rviz:=true
'\'' C-m
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /robot/dlio/odom_node/odom /robot/dlio/odom_node/pointcloud/deskewed -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /ros_ws/devel/setup.bash
    rosrun dlio-to-hdmapping listener \
